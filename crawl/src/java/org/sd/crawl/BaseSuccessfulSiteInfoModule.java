...
/**
 * A base site info module for handling successfully crawled pages.
 * <p>
 * @author Spence Koehler
 */
public abstract class BaseSuccessfulSiteInfoModule extends BaseSiteInfoModule {
  
  /**
   * Do the work of adding information about a successfully crawled
   * page.
   * <p>
   * Here, unlike with the addFetchedPage and addRevisitedPage methods,
   * we are guaranteeing that the detailed url is non-null.
   */
  protected abstract void doAdd(CrawledPage crawledPage, int crawlDepth, DetailedUrl dUrl);


  protected BaseSuccessfulSiteInfoModule() {
    super();
  }
  ...
  public void addFetchedPage(CrawledPage crawledPage, int crawlDepth, DetailedUrl dUrl) {
    doAdd(crawledPage, crawlDepth, dUrl == null ? new DetailedUrl(crawledPage.getUrl()) : dUrl);
  }
  ...
  public void addRevisitedPage(CrawledPage crawledPage, int crawlDepth, DetailedUrl dUrl) {
    ...
    //      but if it does, we will (re)count the query.
    doAdd(crawledPage, crawlDepth, dUrl == null ? new DetailedUrl(crawledPage.getUrl()) : dUrl);
  }
