 ...
 * <p>
 * Works with the SoStringWrapperTokenPointer to deliver only the shortest
 * tokens for a string.
 *
 * @author Spence Koehler
 */
public class SoTokenPointerFactory extends DefaultTokenPointerFactory {
  
  /**
   * Package protected to enforce creating through TokenizationStrategy.
   */
  SoTokenPointerFactory(Lexicon lexicon) {
    super(lexicon);
  }

  protected TokenPointer makeTokenPointer(StringWrapper.SubString subString, int skipUpTo) {
    return new SoStringWrapperTokenPointer(lexicon, subString, skipUpTo);
  }

  public TokenPointer getFirstPointer(StringWrapper stringWrapper, int skipUpTo) {
    TokenPointer result = null;
    final StringWrapper.SubString firstSubString = stringWrapper.getShortestSubString(0);
