...
public class DepthFirstIterator<T> implements TraversalIterator<T> {
  ...
  public Tree<T> next() {
    ...
    if (children != null) queue.addAll(0, children);
    lastNode = result;
    return result;
  }
  ...
  public void skip() {
    if (lastNode != null) {
      // we just added the children to the beginning of the queue, so lop 'em off.
      final List<Tree<T>> children = lastNode.getChildren();
      if (children != null) {
        for (Tree<T> child : children) queue.removeFirst();
      }
    }
  }
}
