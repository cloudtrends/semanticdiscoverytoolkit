 ...
 * revising tokens.
 * <p>
 * This implementation only tries the shortest tokens.
 *
 * @author Spence Koehler
 */
public class SoStringWrapperTokenPointer extends StringWrapperTokenPointer {
  
  SoStringWrapperTokenPointer(Lexicon lexicon, StringWrapper.SubString subString, int skipUpTo) {
  ...
  protected StringWrapper.SubString doGetNext(StringWrapper.SubString subString) {
    return subString.getNextShortestSubString();
  }

  protected final TokenPointer buildNextTokenPointer(StringWrapper.SubString subString, int newSkipLimit) {
    return new SoStringWrapperTokenPointer(getLexicon(), subString, newSkipLimit);
  }
}
